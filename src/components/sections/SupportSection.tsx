    ...
    <Section id="support" variant="gradient" size="lg" ref={sectionRef} showTricolor>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
        <div className={isVisible ? 'animate-fade-up' : 'opacity-0'}>
          ...
          <Heading as="h2" size="section" variant="white" className="mb-6">Richiedi Informazioni</Heading>
          <Text variant="white" size="lg" className="mb-10 opacity-90">Scopri la soluzione LEM CERAMIC più adatta alle tue esigenze. Compila il form e ti risponderemo entro 24 ore lavorative.</Text>

          <div className="space-y-6 mb-10">
            {[
              { icon: <svg className="w-6 h-6 text-[var(--color-accent)]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>, title: 'Email', content: <a href="mailto:info@lemsolutions.it" className="text-white/70 hover:text-[var(--color-accent)] transition-colors">info@lemsolutions.it</a> },
              { icon: <svg className="w-6 h-6 text-[var(--color-accent)]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>, title: 'Telefono', content: <a href="tel:+393474806300" className="text-white/70 hover:text-[var(--color-accent)] transition-colors">+39 347 480 6300</a> },
              { icon: <svg className="w-6 h-6 text-[var(--color-accent)]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>, title: 'Sede', content: <span className="text-white/70">Via Gondar 6, 20900 Monza (MB)</span> },
            ].map((item, i) => (
              <div key={i} className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-lg bg-white/10 flex items-center justify-center flex-shrink-0">{item.icon}</div>
                <div><h4 className="font-semibold text-white mb-1">{item.title}</h4>{item.content}</div>
              </div>
            ))}
          </div>
